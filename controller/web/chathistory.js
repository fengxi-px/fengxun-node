...
class ChathistoryController {
  ...
  async saveChatHistory(ctx) {
    ...
    await validate.arrayCheck(messages, "请上传聊天记录", "messages");
    // 存储会话id
    let session_id = "";
    if (sessionId) {
    ...
    if (sessionId) {
      ...
      const result = await new ChathistoryService(ctx).newChatHistory(
        openid,
        session_id
      );
      ctx.send(result);
  ...
  async onliChatHistory(ctx) {
    ...
    await validate.undefinedCheck(sessionId, "缺少会话id值", "sessionId");
    // 用户openid
    const openid = ctx.auth.uid;
  ...
  // 删除聊天记录
  async deleteChatHistory(ctx) {
    const { sessionId } = ctx.query;
    await validate.undefinedCheck(sessionId, "缺少会话id值", "sessionId");
    // 用户openid
    const openid = ctx.auth.uid;
    const key = `chat:${openid}`;
    const sessionKey = `chat:${openid}:${sessionId}`;
    await ctx.redis.del(sessionKey);
    await ctx.redis.zrem(key, sessionId);
    ctx.send("success");
  }
}
